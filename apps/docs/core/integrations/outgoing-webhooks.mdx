...
description: Send task activity from a TeamOS project to Slack, Discord, Telegram, or any HTTP endpoint. Configure per-project notifications and optional signing.
---

TeamOS can notify external systems when tasks change in a project. Each integration is per project and uses either an incoming webhook URL from the service you connect (Slack, Discord), a bot token plus chat ID (Telegram), or your own HTTP listener (generic webhook).

<Info>
These integrations are outgoing only: TeamOS posts to your URL when events occur. They are separate from [GitHub integration](/core/integrations/github/setup), which syncs issues and repositories.
</Info>
...
Notification messages and generic webhook payloads include links to the task in the web app. Set [`KANEO_CLIENT_URL`](/core/installation/environment-variables) to your public TeamOS web URL so those links point to your instance (for example `https://kaneo.example.com`).
...
TeamOS sends formatted messages to Slack when enabled events occur.
...
After delivery attempts, TeamOS stores basic health metadata (for example last success, last failure message, failure count) so you can see whether recent posts succeeded. This does not replace monitoring on your endpoint.
...
- `timestamp`: ISO 8601 time when TeamOS sent the request
...
If you configure a secret for the generic webhook integration, TeamOS signs each request and sends the result in the `X-Kaneo-Signature` header.

- Header: `X-Kaneo-Signature`
- Algorithm: `HMAC-SHA256`
- Signed input: the exact raw request body TeamOS sends
...
TeamOS currently signs only the raw request body. It does not include a separate timestamp value in the signature input or send a dedicated timestamp header for replay protection. If you add replay defenses in front of your receiver, document that behavior separately so it does not drift from TeamOS's payload contract.
