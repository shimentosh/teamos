---
title: TeamOS MCP Server
description: Connect TeamOS to MCP clients like Cursor and Claude Desktop using the official TeamOS MCP server.
---

TeamOS supports the Model Context Protocol (MCP) in two ways:

1. **Built-in HTTP endpoint**: every TeamOS instance exposes `/api/mcp` out of the box. No extra setup, no extra process.
2. **Stdio package** ([`@kaneo/mcp`](https://www.npmjs.com/package/@kaneo/mcp)): the official TeamOS MCP package on npm, a local stdio server for clients that prefer stdio transport.

Both offer the same tools for managing workspaces, projects, tasks, comments, and labels.

## Built-in HTTP endpoint

Every TeamOS API instance ships with a Streamable HTTP MCP endpoint. This is the recommended approach for self-hosted deployments.
...
On first connect, you will be redirected to TeamOS to sign in and explicitly approve the MCP client. After approval, all MCP tools run as your authenticated user.
...
The `@kaneo/mcp` package runs a local stdio MCP server and authenticates with TeamOS using device authorization. Use this when your MCP client requires stdio transport.

### Prerequisites

- Node.js 24 or newer
- A running TeamOS API
- Access to the TeamOS web app to approve device login

By default, self-hosted TeamOS allows both `kaneo-cli` and `kaneo-mcp` as device auth client IDs.
...
When generating config, pass your TeamOS base URL:
...
| --- | --- |
| `KANEO_API_URL` | TeamOS API origin. Default: `http://localhost:1337`. Do not include `/api`. |
...
On the first tool call that needs TeamOS access, the MCP server:

1. Requests a device code from TeamOS
