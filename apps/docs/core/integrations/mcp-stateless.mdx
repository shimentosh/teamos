...
description: Protocol contract, verification, limitations, and rollback for TeamOS's dual-era Streamable HTTP endpoint.
---
...
TeamOS validates the bearer token before protocol routing on every request.
...
The tests use an in-process handler and stub the internal TeamOS API fetch. They
do not mutate a real TeamOS database.
...
session, inspect `/mcp`, list TeamOS tools, call `whoami`, and perform one
...
With the feature disabled, Codex uses its legacy MCP lifecycle. TeamOS should
...
- The modern handler is request-local. TeamOS tools currently need no
...
Production rollback must happen by deploying the previous TeamOS image through
