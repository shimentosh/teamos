---
title: App Setup
description: Learn how to create and configure a GitHub App for TeamOS integration. Step-by-step guide to register your TeamOS instance with GitHub.
---


This guide walks you through creating and configuring a GitHub App to enable the TeamOS integration. **Start here before deploying or configuring your TeamOS instance.**

## Overview

A GitHub App provides secure, granular access to your repositories and enables powerful bidirectional synchronization between TeamOS and GitHub:

**Task to GitHub:**
- Automatically create GitHub issues when you create tasks
- Update issue status and priority labels when you change tasks
- Add comments linking to TeamOS tasks
...
- Sync GitHub labels to TeamOS task labels
...
<Warning>
**Important**: Complete this GitHub App setup **before** deploying TeamOS. You'll need the app credentials during deployment configuration.
</Warning>
...
- **Description**: `TeamOS project management integration`
- **Homepage URL**: Your planned TeamOS instance URL (e.g., `https://kaneo.yourdomain.com`)
...
<Warning>
Replace `your-kaneo-instance.com` with your planned TeamOS domain. The webhook URL must be publicly accessible once deployed.
</Warning>
</Step>
...
<Warning>
Keep this private key secure! It's used to authenticate your TeamOS instance with GitHub.
</Warning>
</Step>
...
The GitHub App also shows an OAuth **Client ID** and **Client Secret** under "Basic information". TeamOS’s **repository integration** does not use those environment variables; they are only relevant if you enable **Sign in with GitHub** using `GITHUB_OAUTH_CLIENT_ID` / `GITHUB_OAUTH_CLIENT_SECRET` (see [GitHub SSO](/core/social-providers/github)).
...
### App Configuration
- **Use descriptive names**: Make it clear the app is for TeamOS integration
...
Ensure your TeamOS instance is:
...
1. **[Deploy TeamOS](/core)** - Deploy your TeamOS instance with the GitHub App credentials
2. **[Configure GitHub Integration](/core/integrations/github/configuration)** - Complete the integration setup in your deployed TeamOS instance
