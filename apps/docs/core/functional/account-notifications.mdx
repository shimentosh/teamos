...
description: Deliver your personal TeamOS notifications by email, ntfy, Gotify, or a custom webhook with workspace and project scoping
---

TeamOS can keep the normal in-app notification inbox and also forward matching notifications to external channels for the signed-in user.
...
TeamOS already stores in-app notifications in the notification inbox. Account notification delivery adds a separate preference layer on top of that behavior.
...
- create an application in your Gotify server before connecting TeamOS
- paste the base server URL, for example `https://gotify.example.com`
- no extra topic or path is required because TeamOS sends directly to the Gotify message API using the application token
...
When a signing secret is configured, TeamOS sends an `X-Kaneo-Signature` header containing the `HMAC-SHA256` hex digest of the raw JSON body.

## Workspace delivery rules

Below the global channel cards, TeamOS shows workspace delivery rules.
...
V1 forwards the same notification types that currently create inbox notifications in TeamOS.
...
If TeamOS cannot resolve a notification to a workspace or project context, that notification stays inbox-only.

## Reachability and security

For security, TeamOS validates outbound HTTP destinations before saving or sending.
...
### Notifications show in TeamOS but are not delivered
