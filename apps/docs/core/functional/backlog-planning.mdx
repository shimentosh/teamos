...
In TeamOS's data model, backlog items use the `planned` task status. A board column named "Backlog" is just a regular board column, not the same thing as the Backlog view.
