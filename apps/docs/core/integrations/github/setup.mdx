---
title: App Setup
description: Learn how to create and configure a GitHub App for Company OS integration. Step-by-step guide to register your Company OS instance with GitHub.
---


This guide walks you through creating and configuring a GitHub App to enable the Company OS integration. **Start here before deploying or configuring your Company OS instance.**

## Overview

A GitHub App provides secure, granular access to your repositories and enables powerful bidirectional synchronization between Company OS and GitHub:

**Task to GitHub:**
- Automatically create GitHub issues when you create tasks
- Update issue status and priority labels when you change tasks
- Add comments linking to Company OS tasks
- Close/reopen issues based on task status changes

**GitHub to Company OS:**
...
- Sync GitHub labels to Company OS task labels
...
<Warning>
**Important**: Complete this GitHub App setup **before** deploying Company OS. You'll need the app credentials during deployment configuration.
</Warning>
...
- **Description**: `Company OS project management integration`
- **Homepage URL**: Your planned Company OS instance URL (e.g., `https://kaneo.yourdomain.com`)
...
<Warning>
Replace `your-kaneo-instance.com` with your planned Company OS domain. The webhook URL must be publicly accessible once deployed.
</Warning>
</Step>
...
<Warning>
Keep this private key secure! It's used to authenticate your Company OS instance with GitHub.
</Warning>
</Step>
...
The GitHub App also shows an OAuth **Client ID** and **Client Secret** under "Basic information". Company OS’s **repository integration** does not use those environment variables; they are only relevant if you enable **Sign in with GitHub** using `GITHUB_OAUTH_CLIENT_ID` / `GITHUB_OAUTH_CLIENT_SECRET` (see [GitHub SSO](/core/social-providers/github)).
...
### App Configuration
- **Use descriptive names**: Make it clear the app is for Company OS integration
...
Ensure your Company OS instance is:
...
1. **[Deploy Company OS](/core)** - Deploy your Company OS instance with the GitHub App credentials
2. **[Configure GitHub Integration](/core/integrations/github/configuration)** - Complete the integration setup in your deployed Company OS instance
