---
title: Slack
description: Send task notifications from Company OS to a Slack channel using incoming webhooks.
---

Company OS can post task activity to a Slack channel whenever events occur in a project. This uses Slack's incoming webhook feature, so no bot token or OAuth flow is required.

## Create a Slack webhook

1. Go to [api.slack.com/apps](https://api.slack.com/apps) and click **Create New App** then **From scratch**.
2. Name your app (e.g. "Company OS") and select your workspace.
...
Company OS sends Slack Block Kit messages with:
