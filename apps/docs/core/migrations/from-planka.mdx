...
description: Move your PLANKA boards into Company OS with the @kaneo/planka-import CLI, including lists, cards, labels, assignees, checklists and comments
---
...
recreates them in Company OS through Company OS's public API.
...
- A PLANKA account that can see the boards you want to move. If you sign in
  ...
  `--planka-api-key`.
- A Company OS workspace, and an API key from **Settings → API keys**.
- Node.js 20 or newer.

Invite your team to the Company OS workspace **before** importing. Assignees are
matched by email address, so anyone who isn't a member yet will have their cards
imported unassigned.

## Start with a dry run

A dry run reads PLANKA only and writes nothing. It needs no Company OS API key, so
...
Planned import:
  • Marketing Site  3 columns  24 tasks  5 labels  12 comments
      ! List "Planned" was imported as column "Planned list" to avoid a naming conflict in Company OS.
...
When the plan looks right, add your Company OS credentials and drop `--dry-run`:
...
Self-hosting Company OS? Point `--kaneo-url` at your own instance. If you leave
...
One Company OS project is created per PLANKA **board**, because the board is what
holds the lists and cards. When a PLANKA project has more than one board, each
resulting Company OS project is named `Project - Board`.
...
- **Comment authorship.** The comment is created by the API key's owner, but the
  original PLANKA author is recorded on it and shown in Company OS, the same way
  imported GitHub comments are. This needs Company OS 2.17.6 or newer; against older
...
- **Priorities.** PLANKA has no priority field, so every task starts at
  `no-priority`.
- **Custom fields, stopwatches, and card subscriptions.** No Company OS equivalent.
...
import and the summary names the one that failed. Delete the half-created Company OS
...
| `--kaneo-url <url>` | Company OS instance URL (default `https://cloud.kaneo.app`) |
| `--kaneo-api-key <key>` | Company OS API key, or set `KANEO_API_KEY` |
