---
title: Gitea Setup
description: Connect TeamOS to a Gitea instance (issues, pull requests, webhooks).
---

## Overview

The Gitea integration links a TeamOS project to a repository on your Gitea instance. Tasks sync with issues, and webhooks keep TeamOS updated when issues, pull requests, or pushes change.
...
3. Copy the token and store it securely; paste it into TeamOS’s Gitea integration settings.

## Webhook in Gitea

TeamOS verifies incoming webhooks with a **HMAC SHA256** secret stored per integration.

### Where to create the webhook in Gitea

1. Log in to Gitea and **open the same repository** you linked in TeamOS (`owner/repo`).
...
1. Connect the repository in TeamOS **Project → Integrations → Gitea** and save.
2. Copy the **Webhook URL** and **Secret** shown in TeamOS (after saving).
3. In Gitea: **Add Webhook → Gitea**.
4. Set **Payload URL** to the TeamOS webhook URL.
5. Set **Secret** to the exact value from TeamOS.
...
8. Save and use **Test Delivery** to confirm a `200` response from TeamOS.
