...
All API endpoints require authentication. Company OS currently supports two authentication patterns for API access:
...
Use API keys when you control secret storage and want a stable credential for a single Company OS instance.
...
The device flow follows RFC 8628 and returns a Bearer token that can be used against that Company OS instance's API.
...
Sign in to your Company OS instance using your account credentials.
</Step>
...
By default, self-hosted Company OS allows the built-in device clients `kaneo-cli` and `kaneo-mcp`.
...
2. Company OS returns a `device_code`, `user_code`, polling interval, and verification URL
...
While waiting for approval, Company OS may return:
...
Once approved, Company OS returns an access token:
