...
impl Tracker {
    ...
    pub fn connect(&self, address: &str, code: &str) -> Result<Status, String> {
        ...
        if api::normalize_code(code).len() < 8 {
            return Err("Enter the pairing code shown in TeamOS.".into());
        }
        let paired = Client::new(&base)
            .pair(code, &device_name())
            .map_err(|e| match e {
                ApiError::Status(404, _) => format!(
                    "No TeamOS server answered at {base}. If the API has its own address, enter that instead."
                ),
                e => e.to_string(),
            })?;
    ...
    pub fn sync_once(&self) -> Result<SyncReport, ApiError> {
        ...
        let result = self.sync_inner(&mut report);
        {
            ...
            match &result {
                ...
                Err(ApiError::Network(e)) => {
                    s.offline = true;
                    s.last_error = Some(format!("Could not reach TeamOS: {e}"));
                }
        ...
        if let Err(ApiError::Unauthorized) = result {
            self.forget(Some(
                "This computer was disconnected in TeamOS. Pair it again to resume.".into(),
            ));
        }
        self.notify();
        result.map(|()| report)
            ...
            let outcome = if valid.is_empty() {
                ...
                match client.upload(&token, &valid) {
                    ...
                    Err(ApiError::Status(400, body)) => {
                        let mut s = self.lock();
                        s.last_error = Some(format!("TeamOS rejected some activity: {body}"));
                        Ok(None)
                    }
                    Err(e) => Err(e),
                }
