...
Company OS supports private uploads in task descriptions and task comments.

Uploads use an S3-compatible object storage backend:

- the Company OS API creates presigned upload URLs
- the browser uploads files directly to the storage backend
- Company OS finalizes uploads into private asset records
- Company OS serves uploaded files back through its own API

This means Company OS can work with multiple backends as long as they expose a compatible S3-style API.
...
When Company OS generates a presigned upload URL, the browser uploads directly to your storage backend.
...
If your Company OS deployment is public, do not leave `S3_ENDPOINT` set to `http://minio:9000`.
...
- `cloud.kaneo.app` for Company OS
...
- Company OS signs uploads against `S3_ENDPOINT`
...
Reads do not need a public bucket URL, because Company OS serves uploaded assets back through `/api/asset/:id`.

## Other S3-compatible backends

Company OS is not tied to MinIO. Any deployment can point Company OS at another S3-compatible backend by changing the `S3_*` environment variables.
...
`fs` is another S3-compatible option if you want a lightweight local object store. It is built by friends of Company OS.

When using `fs`, configure Company OS like this:
...
- Company OS still expects the bucket to exist before uploads start
- direct browser uploads may need reverse-proxy CORS headers, because `fs` does not currently implement S3 CORS APIs

## Required Company OS storage variables

Set these in your Company OS deployment:

| Name | Description |
| --- | --- |
| `S3_ENDPOINT` | S3-compatible API endpoint used by Company OS |
...
| Name | Description |
| --- | --- |
| `S3_PUBLIC_BASE_URL` | Optional public asset base URL. Company OS does not require this for the current private asset flow. |
...
When Company OS runs on AWS (EC2, ECS, or EKS), you can let it authenticate with an
IAM role instead of long-lived access keys.

Leave both `S3_ACCESS_KEY_ID` and `S3_SECRET_ACCESS_KEY` unset. Company OS then falls
...
<Warning>
  ...
  `S3_SECRET_ACCESS_KEY` is treated as a misconfiguration and Company OS will refuse
