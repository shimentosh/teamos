# TeamOS Agent connector (Chrome / Edge)

A Manifest V3 extension that tells the TeamOS Agent on the same Windows computer which **website domain** is open in the active tab. Without it, the agent on Windows records the browser as an application but no domain. macOS does not need it: the agent reads Safari and Chrome there through AppleScript.
...
1. Install and start the TeamOS Agent once. On start it registers the native messaging host for Chrome, Edge and Brave (per user, under `HKCU`).
