...
Out of scope: findings that require an already-compromised host or database, denial of service through sheer request volume, missing hardening headers with no demonstrated impact, and vulnerabilities in third-party dependencies without a working path through TeamOS. For dependency advisories, a pull request bumping the dependency is welcome and can be public.

Self-hosted deployments are configured by their operator. Reports that depend on an insecure configuration are useful when TeamOS's own defaults or documentation lead operators there; please say which default you followed.
