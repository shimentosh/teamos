...
description: Protocol contract, verification, limitations, and rollback for Company OS's dual-era Streamable HTTP endpoint.
---
...
Company OS validates the bearer token before protocol routing on every request.
...
The tests use an in-process handler and stub the internal Company OS API fetch. They
do not mutate a real Company OS database.
...
session, inspect `/mcp`, list Company OS tools, call `whoami`, and perform one
...
With the feature disabled, Codex uses its legacy MCP lifecycle. Company OS should
...
- The modern handler is request-local. Company OS tools currently need no
...
Production rollback must happen by deploying the previous Company OS image through
