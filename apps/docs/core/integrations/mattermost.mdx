---
title: Mattermost
description: Send task notifications from TeamOS to a Mattermost channel using incoming webhooks.
---

TeamOS can post task activity to a Mattermost channel whenever events occur in a project. This uses Mattermost's incoming webhook feature — no bot account or OAuth flow required.

## Create a Mattermost webhook

1. In Mattermost, go to **Main Menu → Integrations → Incoming Webhooks**.
2. Click **Add Incoming Webhook**.
3. Give it a title (e.g. "TeamOS") and select the default channel where notifications should be posted.
...
TeamOS sends messages using Mattermost's legacy attachment format with:

- A clickable title linking to the task in TeamOS
