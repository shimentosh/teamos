...
This guide explains how to deploy Company OS using Nginx as a reverse proxy. This setup is ideal for production environments where you want to expose Company OS through a domain name with HTTPS.
...
The next step is to configure Nginx to proxy requests to the Company OS services. We will create two servers, one for the web application and one for the API.
