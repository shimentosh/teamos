---
title: Discord
description: Send task notifications from TeamOS to a Discord channel using incoming webhooks.
---

TeamOS can post task activity to a Discord channel whenever events occur in a project. This uses Discord's incoming webhook feature, so no bot or OAuth setup is required.
...
4. Click **New Webhook**.
5. Give the webhook a name (e.g. "TeamOS") and optionally set an avatar.
...
TeamOS sends Discord embed messages with:

- A title describing the event (e.g. "Task Status Changed")
- Task name with a clickable link to the task in TeamOS
