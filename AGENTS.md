...
- `apps/desktop` — Tauri shell that loads the instance in a desktop window (browser sign-in, OS notifications, tray).
- `apps/agent` — separate, optional Tauri app that reports activity to a workspace. Both stay outside the pnpm workspace.
...
Deployment is Dokploy, not GitHub. This repository has **no CI**: no workflows,
no image publishing, no release automation. A push to `main` fires a Dokploy
webhook, and Dokploy's build server builds `Dockerfile.kaneo` through
`compose.dokploy.yml` and restarts the container. See
[deploy/dokploy/README.md](./deploy/dokploy/README.md).

Because nothing runs checks for you, run them before you hand work over:
`pnpm typecheck`, `pnpm test`, `pnpm i18n:check`, `pnpm openapi:check`.

`scripts/release/` and `release.config.js` are left over from the removed
release workflow. Conventional Commits still make the history readable, but no
version is bumped and no tag is cut on their own.

The desktop app in `apps/desktop` is built locally and uploaded to R2; it is not
part of the server deploy.
