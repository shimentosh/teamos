...
TeamOS uses the following services:
...
For this Compose setup, the bundled TeamOS container reaches PostgreSQL at the service hostname `postgres`.
If you run the API directly on your host instead, use `localhost` or set `DATABASE_URL` explicitly.

## Notes on object storage

TeamOS uses S3-compatible object storage for private uploads in task descriptions and comments.

- Object storage is optional. TeamOS runs without it, but uploads in task descriptions and comments will be unavailable.
- For local/self-hosted setups, MinIO is the recommended option when you want uploads.
- The TeamOS API creates presigned upload URLs and the browser uploads files directly to the configured storage backend.
- TeamOS serves uploaded assets back through its own API, so the bucket does not need to be public.
...
If you want to use [`fs`](https://github.com/ferdzo/fs), you can point the same `S3_*` variables at its endpoint instead of MinIO. TeamOS only needs an S3-compatible API endpoint, credentials, and a bucket.
