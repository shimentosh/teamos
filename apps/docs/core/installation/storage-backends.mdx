...
TeamOS uses S3-compatible object storage for uploads in:
...
TeamOS then serves uploaded assets back through its own API.
...
This works when TeamOS and MinIO are on the same Docker network and your browser reaches MinIO through `localhost`.
...
- MinIO CORS allowing your TeamOS origin
...
If TeamOS runs on AWS (EC2, ECS, or EKS), you can omit `S3_ACCESS_KEY_ID` and
`S3_SECRET_ACCESS_KEY` entirely and let TeamOS authenticate with the instance's
...
- a public bucket is not required for TeamOS's current private asset flow

## One copy-paste self-hosted example

This example gives you TeamOS + MinIO in one Compose file.
...
2. Configure MinIO CORS for your TeamOS origin.
