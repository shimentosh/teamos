...
Company OS uses S3-compatible object storage for uploads in:
...
Company OS then serves uploaded assets back through its own API.
...
This works when Company OS and MinIO are on the same Docker network and your browser reaches MinIO through `localhost`.
...
- Company OS: `https://cloud.kaneo.app`
...
- MinIO CORS allowing your Company OS origin
...
If Company OS runs on AWS (EC2, ECS, or EKS), you can omit `S3_ACCESS_KEY_ID` and
`S3_SECRET_ACCESS_KEY` entirely and let Company OS authenticate with the instance's
...
- a public bucket is not required for Company OS's current private asset flow

## One copy-paste self-hosted example

This example gives you Company OS + MinIO in one Compose file.
...
2. Configure MinIO CORS for your Company OS origin.
