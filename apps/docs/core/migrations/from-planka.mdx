...
description: Move your PLANKA boards into TeamOS with the @kaneo/planka-import CLI, including lists, cards, labels, assignees, checklists and comments
---
...
recreates them in TeamOS through TeamOS's public API.
...
- A PLANKA account that can see the boards you want to move. If you sign in
  ...
  `--planka-api-key`.
- A TeamOS workspace, and an API key from **Settings → API keys**.
- Node.js 20 or newer.

Invite your team to the TeamOS workspace **before** importing. Assignees are
matched by email address, so anyone who isn't a member yet will have their cards
imported unassigned.

## Start with a dry run

A dry run reads PLANKA only and writes nothing. It needs no TeamOS API key, so
...
Planned import:
  • Marketing Site  3 columns  24 tasks  5 labels  12 comments
      ! List "Planned" was imported as column "Planned list" to avoid a naming conflict in TeamOS.
...
When the plan looks right, add your TeamOS credentials and drop `--dry-run`:
...
Self-hosting TeamOS? Point `--kaneo-url` at your own instance. If you leave
...
One TeamOS project is created per PLANKA **board**, because the board is what
holds the lists and cards. When a PLANKA project has more than one board, each
resulting TeamOS project is named `Project - Board`.
...
- **Comment authorship.** The comment is created by the API key's owner, but the
  original PLANKA author is recorded on it and shown in TeamOS, the same way
  imported GitHub comments are. This needs TeamOS 2.17.6 or newer; against older
...
- **Priorities.** PLANKA has no priority field, so every task starts at
  `no-priority`.
- **Custom fields, stopwatches, and card subscriptions.** No TeamOS equivalent.
...
import and the summary names the one that failed. Delete the half-created TeamOS
...
| `--kaneo-url <url>` | TeamOS instance URL (default `https://cloud.kaneo.app`) |
| `--kaneo-api-key <key>` | TeamOS API key, or set `KANEO_API_KEY` |
