...
All API endpoints require authentication. TeamOS currently supports two authentication patterns for API access:
...
Use API keys when you control secret storage and want a stable credential for a single TeamOS instance.
...
The device flow follows RFC 8628 and returns a Bearer token that can be used against that TeamOS instance's API.
...
Sign in to your TeamOS instance using your account credentials.
</Step>
...
By default, self-hosted TeamOS allows the built-in device clients `kaneo-cli` and `kaneo-mcp`.
...
2. TeamOS returns a `device_code`, `user_code`, polling interval, and verification URL
...
While waiting for approval, TeamOS may return:
...
Once approved, TeamOS returns an access token:
