//! Client for the TeamOS device endpoints (`/api/agent/device/*`).
...
#[derive(Debug)]
pub enum ApiError {
    /// 401: the device was revoked in TeamOS.
    Unauthorized,
...
impl std::fmt::Display for ApiError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ApiError::Unauthorized => write!(f, "This computer was disconnected in TeamOS."),
            ApiError::Status(code, body) if body.trim().is_empty() => {
                write!(f, "TeamOS answered with status {code}.")
            }
            ApiError::Status(_, body) => write!(f, "{}", body.trim()),
            ApiError::Network(e) => write!(f, "Could not reach TeamOS: {e}"),
        }
    }
}
...
pub fn api_base(input: &str) -> Result<String, String> {
    ...
    if trimmed.is_empty() {
        return Err("Enter your TeamOS address.".into());
    }
