---
title: Coolify
description: Deploy Company OS to Coolify using the bundled Docker Compose file.
---

## Coolify

Company OS ships a [Coolify-optimized Docker Compose file](https://github.com/usekaneo/kaneo/blob/main/compose.coolify.yml) so you can deploy the pre-built `ghcr.io/usekaneo/kaneo` image from the repository without building anything.
...
5. **Set environment variables** (pre-filled from the compose file):
   ...
   - `KANEO_IMAGE_TAG` (optional): the Company OS image version to deploy. It defaults to `latest`; set it to a release tag such as `2.20.0` to hold the deployment on a known version.
6. **Deploy**.

## What the Compose File Sets Up

- **PostgreSQL 16** with a persistent volume; Company OS waits for its health check before starting.
- **Company OS bundled image** (nginx + API in one container) on port `5173`.
- **Company OS version**: the image tracks `latest`, so a redeploy picks up the newest release. The API runs database migrations on startup, so set `KANEO_IMAGE_TAG` to a release tag if you would rather upgrade deliberately.
