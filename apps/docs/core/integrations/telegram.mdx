---
title: Telegram
description: Send task notifications from TeamOS to a Telegram chat or topic using a bot token and chat ID.
---

TeamOS can post task activity to Telegram whenever events occur in a project. Unlike Slack and Discord, Telegram uses a bot token plus a chat ID instead of an incoming webhook URL.
...
TeamOS sends formatted Telegram messages with:

- The event title
- A direct link to the task in TeamOS
- Project, status, and priority details
- The name of the user who triggered the event when available

Make sure [`KANEO_CLIENT_URL`](/core/installation/environment-variables) is set to your public TeamOS URL so task links work correctly.

## Updating credentials

To rotate the bot token after connecting, paste a new token in the bot token field and click **Save Changes**. If you leave the token field empty, TeamOS keeps the currently stored token.
