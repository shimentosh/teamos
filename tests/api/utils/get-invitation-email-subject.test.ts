...
describe("getInvitationEmailSubject", () => {
  it("uses French copy for regional French locales", () => {
    ...
    expect(subject).toBe(
      "Alice vous invite à rejoindre Équipe produit sur Company OS",
    );
  });
  ...
  it("keeps the German translation unchanged", () => {
    ...
    expect(subject).toBe(
      "Alice hat dich eingeladen, Produkt auf Company OS beizutreten",
    );
  });
  ...
  it("uses Brazilian Portuguese copy for regional Portuguese locales", () => {
    ...
    expect(subject).toBe(
      "Alice convidou você para participar de Equipe produto no Company OS",
    );
  });
  ...
  it("uses Japanese copy for Japanese locales", () => {
    ...
    expect(subject).toBe(
      "Alice さんが Company OS の「プロダクト」にあなたを招待しました",
    );
  });
  ...
  it("uses the English fallback for unsupported locales", () => {
    ...
    expect(subject).toBe("Alice invited you to join Producto on Company OS");
  });
});
