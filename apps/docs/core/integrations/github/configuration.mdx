...
description: Configure Company OS with your GitHub App credentials and connect projects to repositories. Includes Docker Compose and Kubernetes examples.
---


This guide covers configuring Company OS with your GitHub App credentials and connecting projects to repositories.
...
Add the following environment variables to your Company OS deployment:
...
1. Open your Company OS project
...
1. Company OS will verify that your GitHub App has access
...
Create a new task in your connected Company OS project with:
- A clear title
- A description
...
- A comment links back to the Company OS task
</Step>
...
1. Change task priority in Company OS → Check issue labels in GitHub
2. Add/remove labels in GitHub → Check task in Company OS
3. Change task status in Company OS → Check if issue is closed/reopened
</Step>
...
Configure how Company OS matches branches to tasks in your project's GitHub integration settings:
...
Company OS automatically creates and syncs these label types:
...
**Custom Labels:**
- Any labels added to GitHub issues (not starting with `priority:` or `status:`) are synced to Company OS
- Labels added in Company OS can be synced to GitHub (if configured)
...
3. Each Company OS project can connect to any repository where your app is installed
