...
description: Move an existing TeamOS deployment onto the drim CLI, or update an older drim deployment to the current single-container layout
---
...
TeamOS used to ship as two images, `api` and `web`, behind a reverse proxy that split traffic between them. It now ships as one container that serves the web app and the API together.
...
You want every service up, the TeamOS container reporting healthy once its start period elapses, and `{"status":"ok"}` from the health endpoint.
...
## Moving an existing TeamOS onto drim

If you deployed TeamOS by hand and want drim to manage it, the goal is to have drim generate its files and then point them at the volumes you already have.
...
If you already run nginx, Traefik or similar, choose `--no-reverse-proxy` at setup so drim does not start Caddy on ports 80 and 443, and point your proxy at TeamOS on port 5173.
