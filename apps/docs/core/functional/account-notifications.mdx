...
description: Deliver your personal Company OS notifications by email, ntfy, Gotify, or a custom webhook with workspace and project scoping
---

Company OS can keep the normal in-app notification inbox and also forward matching notifications to external channels for the signed-in user.
...
Company OS already stores in-app notifications in the notification inbox. Account notification delivery adds a separate preference layer on top of that behavior.
...
- create an application in your Gotify server before connecting Company OS
- paste the base server URL, for example `https://gotify.example.com`
- no extra topic or path is required because Company OS sends directly to the Gotify message API using the application token
...
When a signing secret is configured, Company OS sends an `X-Kaneo-Signature` header containing the `HMAC-SHA256` hex digest of the raw JSON body.

## Workspace delivery rules

Below the global channel cards, Company OS shows workspace delivery rules.
...
V1 forwards the same notification types that currently create inbox notifications in Company OS.
...
If Company OS cannot resolve a notification to a workspace or project context, that notification stays inbox-only.

## Reachability and security

For security, Company OS validates outbound HTTP destinations before saving or sending.
...
### Notifications show in Company OS but are not delivered
