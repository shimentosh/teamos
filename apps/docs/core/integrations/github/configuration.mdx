...
description: Configure TeamOS with your GitHub App credentials and connect projects to repositories. Includes Docker Compose and Kubernetes examples.
---


This guide covers configuring TeamOS with your GitHub App credentials and connecting projects to repositories.
...
Add the following environment variables to your TeamOS deployment:
...
1. TeamOS will verify that your GitHub App has access
...
Create a new task in your connected TeamOS project with:
- A clear title
- A description
...
- A comment links back to the TeamOS task
</Step>
...
1. Change task priority in TeamOS → Check issue labels in GitHub
2. Add/remove labels in GitHub → Check task in TeamOS
3. Change task status in TeamOS → Check if issue is closed/reopened
</Step>
...
Configure how TeamOS matches branches to tasks in your project's GitHub integration settings:
...
TeamOS automatically creates and syncs these label types:
...
**Custom Labels:**
- Any labels added to GitHub issues (not starting with `priority:` or `status:`) are synced to TeamOS
- Labels added in TeamOS can be synced to GitHub (if configured)
...
3. Each TeamOS project can connect to any repository where your app is installed
